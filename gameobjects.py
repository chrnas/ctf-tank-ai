...
class Tank(GamePhysicsObject):
    ...
    def try_grab_flag(self, flag):
        ...
        if(not flag.is_on_tank):
            ...
            if((flag_pos - self.body.position).length < 0.5):
                ...
                #Sound for Flag
                #sound_flag_captured = pygame.mixer.Sound("data//alert.wav") 
                #sound_flag_captured.set_volume(1)
                #sound_flag_captured.play()
    ...
    #def move_flag_back(self, flag):
     #   flag.is_on_tank   = False

    def shoot(self, space):
        """ Call this function to shoot a missile (current implementation does nothing ! you need to implement it yourself) """
        if self.shoot_time > 50:
            self.shoot_time = 0
            return Bullet(self.body.position[0] - (math.sin(self.body.angle)/2), self.body.position[1] + (math.cos(self.body.angle)/2), math.degrees(self.body.angle), space)
        else:
            return False
...
class Flag(GameVisibleObject):
    ...
    def move_flag_back(self, x, y):
        self.x = x
        self.y = y

class Bullet(GamePhysicsObject):
    """This class explains the path of the bullet"""
    ACCELERATION = 0.4
    NORMAL_MAX_SPEED = 10
    ...
    def __init__(self, x, y, orientation, space):
        ...
        # Define variable used to apply motion to the tanks
        self.acceleration = 5 # 1 forward, 0 for stand still, -1 for backwards
